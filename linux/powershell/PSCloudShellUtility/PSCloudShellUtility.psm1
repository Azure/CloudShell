...
function Enable-PSReadLinePrediction {
    # PSReadline module may be unloaded or gets deleted from user profiles
    if (-not (Microsoft.PowerShell.Core\Get-Module -Name PSReadline)) {
        return
    }
    ...
    if ($windowHeight -lt 15 -or $windowWidth -lt 54) {
        PSReadline\Set-PSReadLineOption -PredictionViewStyle InlineView
        PSReadline\Set-PSReadLineOption -Colors @{InlinePrediction = 'Blue'}
    }
    else {
        PSReadline\Set-PSReadLineOption -PredictionViewStyle ListView
    }
}
